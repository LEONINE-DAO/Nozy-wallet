...
impl BlockParser {
    ...
    pub async fn parse_block(&self, block_height: u32) -> NozyResult<Vec<ParsedTransaction>> {
        ...
        let json_string = serde_json::to_string(&block_data)
    ...
    pub fn parse_block_data(&self, block_data: &str) -> NozyResult<Vec<ParsedTransaction>> {
        let json_data: Value = serde_json::from_str(block_data)
            .map_err(|e| NozyError::InvalidOperation(format!("Failed to parse block JSON: {}", e)))?;
        
        let transactions = json_data["result"]["transactions"]
            .as_array()
            .ok_or_else(|| NozyError::InvalidOperation("No transactions found in block".to_string()))?;
    ...
    pub fn parse_orchard_action(&self, action_data: &Value) -> NozyResult<OrchardAction> {
        let action_type = if action_data["nullifier"].is_string() && !action_data["nullifier"].as_str().unwrap_or("").is_empty() {
            ActionType::Spend
        } else {
            ActionType::Output
        };
        ...
        Ok(OrchardAction {
            ...
            encrypted_note: action_data["encrypted_note"].as_str().map(|s| s.to_string()),
            ...
            encrypted_ciphertext: action_data["encrypted_ciphertext"].as_str().map(|s| s.to_string()),
            out_ciphertext: action_data["out_ciphertext"].as_str().map(|s| s.to_string()),
        })
    }
    ...
    pub fn parse_sapling_spend(&self, spend_data: &Value) -> NozyResult<SaplingSpend> {
        Ok(SaplingSpend {
            ...
            spend_auth_sig: spend_data["spend_auth_sig"].as_str().unwrap_or("").to_string(),
        })
    }
    ...
    pub fn parse_sapling_output(&self, output_data: &Value) -> NozyResult<SaplingOutput> {
        Ok(SaplingOutput {
            ...
            ephemeral_key: output_data["ephemeral_key"].as_str().unwrap_or("").to_string(),
            enc_ciphertext: output_data["enc_ciphertext"].as_str().unwrap_or("").to_string(),
            out_ciphertext: output_data["out_ciphertext"].as_str().unwrap_or("").to_string(),
    ...
    pub fn parse_transparent_input(&self, input_data: &Value) -> NozyResult<TransparentInput> {
        Ok(TransparentInput {
            ...
            script_sig: input_data["scriptSig"]["asm"].as_str().unwrap_or("").to_string(),
    ...
    pub fn parse_transparent_output(&self, output_data: &Value) -> NozyResult<TransparentOutput> {
        ...
        Ok(TransparentOutput {
            value: value as u64,
            script_pubkey: output_data["scriptPubKey"]["asm"].as_str().unwrap_or("").to_string(),
        })
    }

    pub fn extract_orchard_notes(&self, _transactions: &[ParsedTransaction], _target_address: &str) -> Vec<crate::notes::OrchardNote> {
