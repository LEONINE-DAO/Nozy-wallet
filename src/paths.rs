...
/// Per-thread wallet data dir override (used by `nozy-ffi` on mobile and by tests).
/// Thread-local so parallel unit tests do not clobber each other's data roots.
thread_local! {
    static WALLET_DATA_DIR_OVERRIDE: RefCell<Option<PathBuf>> = const { RefCell::new(None) };
}
...
pub fn with_wallet_data_dir<T>(dir: impl AsRef<Path>, f: impl FnOnce() -> T) -> T {
    ...
    WALLET_DATA_DIR_OVERRIDE.with(|cell| {
...
pub fn get_wallet_data_dir() -> PathBuf {
    if let Some(dir) = WALLET_DATA_DIR_OVERRIDE.with(|cell| cell.borrow().clone()) {
        return dir;
    }
