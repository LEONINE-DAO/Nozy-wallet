use clap::Parser;
use nozy::{HDWallet, WalletStorage, ZebraClient, NoteScanner, load_config};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let start_height = args.start_height.unwrap_or_else(|| {
        current_height.saturating_sub(args.test_blocks)
    });
    let end_height = start_height + args.test_blocks - 1;
    
    println!("🧪 Testing sync speed on {} blocks ({} to {})", 
    ...
    println!("  Blocks to scan: {} (from {} to {})", 
    ...
    println!("  ⚡ Optimistic estimate (fetch only): {}", format_duration(optimistic_time));
    ...
    if !parse_times.is_empty() {
        ...
        let realistic_time = Duration::from_secs_f64(blocks_to_scan as f64 * full_scan_time_per_block);
        println!("  🎯 Realistic estimate (with decryption): {}", format_duration(realistic_time));
    }
    
    // Estimate for incremental sync (last 1000 blocks)
    let incremental_blocks = 1000u32;
    let incremental_time = Duration::from_secs_f64(incremental_blocks as f64 * avg_total.as_secs_f64());
    println!("\nFor INCREMENTAL sync (last {} blocks):", incremental_blocks);
