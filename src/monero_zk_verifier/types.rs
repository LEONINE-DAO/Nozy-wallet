...
impl VerificationLevel {
    ...
    pub fn description(&self) -> &'static str {
        match self {
            ...
            VerificationLevel::VerifyBlock { .. } => "Verify single block with ZK proof (1-2 hours)",
            VerificationLevel::VerifyChain { .. } => "Verify block chain with ZK proofs (multiple hours)",
        }
    }
...
impl VerificationResult {
    pub fn success(level: VerificationLevel, verification_time: u64, proof_path: Option<PathBuf>) -> Self {
        Self {
            verified: true,
            level,
