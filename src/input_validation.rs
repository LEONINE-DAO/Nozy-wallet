...
pub fn validate_zcash_address(address: &str) -> NozyResult<()> {
    if address.is_empty() {
        return Err(NozyError::InvalidInput("Address cannot be empty".to_string()));
    }
    ...
    if address.len() < 78 || address.len() > 140 {
        return Err(NozyError::InvalidInput(
            format!("Invalid address length: {} (expected 78-140 characters)", address.len())
        ));
    }
...
pub fn validate_amount(amount: f64, min: f64, max: f64, unit: &str) -> NozyResult<()> {
    if amount <= 0.0 {
        return Err(NozyError::InvalidInput(
            format!("Amount must be greater than zero")
        ));
    }
    
    if amount < min {
        return Err(NozyError::InvalidInput(
            format!("Amount too small: minimum is {} {}", min, unit)
        ));
    }
    
    if amount > max {
        return Err(NozyError::InvalidInput(
            format!("Amount too large: maximum is {} {}", max, unit)
        ));
    }
...
pub fn validate_memo(memo: &str) -> NozyResult<()> {
    ...
    if memo_bytes.len() > 512 {
        return Err(NozyError::InvalidInput(
            format!("Memo too long: {} bytes (maximum 512)", memo_bytes.len())
        ));
    }
