...
use crate::privacy_network::proxy::{PrivacyProxy, PrivacyNetwork};
use reqwest::Client;
...
impl PrivacyProxy for I2PProxy {
    ...
    fn create_client(&self) -> NozyResult<Client> {
        Client::builder()
            .proxy(
                reqwest::Proxy::http(&self.proxy_url)
                    .map_err(|e| NozyError::NetworkError(format!("Invalid I2P proxy URL: {}", e)))?
            )
