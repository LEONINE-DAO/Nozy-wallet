...
impl MoneroRpcClient {
    ...
    pub async fn call(&self, method: &str, params: Value) -> NozyResult<Value> {
        ...
        if let Some(error) = json.get("error") {
            return Err(NozyError::NetworkError(format!("Monero RPC error: {}", error)));
        }
    ...
    pub async fn create_address(&self, account_index: u32) -> NozyResult<String> {
        let result = self.call("create_address", json!({
    ...
    pub async fn send(
        ...
        let txid = result.get("tx_hash")
            .and_then(|v| v.as_str())
            .ok_or_else(|| NozyError::NetworkError("Invalid transaction response".to_string()))?
            .to_string();
        
        let tx_key = result.get("tx_key")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        
        let fee_atomic = result.get("fee")
            .and_then(|v| v.as_u64());
    ...
    pub async fn get_block_hash(&self, height: u64) -> NozyResult<String> {
        let result = self.call("get_block_hash", json!({
            "height": height
        })).await?;
