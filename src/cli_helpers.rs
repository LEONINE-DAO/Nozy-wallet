        ...
        Ok(estimate) => {
            ...
            if let Some(fee_value) = estimate.get("fee") {
                if let Some(fee_f64) = fee_value.as_f64() {
                    ...
                    println!("💰 Estimated fee: {:.8} ZEC ({})", fee_u64 as f64 / 100_000_000.0, fee_u64);
                    return fee_u64;
                }
            }
            ...
            println!("⚠️  Could not parse fee estimate, using default: {:.8} ZEC", DEFAULT_FEE_ZATOSHIS as f64 / 100_000_000.0);
            DEFAULT_FEE_ZATOSHIS
        }
        Err(e) => {
            println!("⚠️  Fee estimation failed: {}, using default: {:.8} ZEC", e, DEFAULT_FEE_ZATOSHIS as f64 / 100_000_000.0);
            DEFAULT_FEE_ZATOSHIS
        }
    }
}
...
pub async fn build_and_broadcast_transaction(
    ...
    use crate::transaction_history::{SentTransactionStorage, SentTransactionRecord};
