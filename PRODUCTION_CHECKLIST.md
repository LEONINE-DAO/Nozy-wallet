...
| **`api-server` (companion)** | P1 | Partial | Seed policy + release packaging in progress: [`SEED_POLICY.md`](api-server/SEED_POLICY.md), [`PARITY.md`](api-server/PARITY.md) (field sign-off open), [`scripts/smoke-companion.ps1`](api-server/scripts/smoke-companion.ps1), [`release.yml`](.github/workflows/release.yml) `build-api` enabled. Bind localhost by default; API key required for `0.0.0.0`. |
