...
| **`api-server` (companion)** | P0 | Partial | Engineering ready for same-machine use: seed policy, CI smoke, `NOZY_PRODUCTION`⇒API key, release assets. **Public GA claim deferred** — see GRANT §2 “when to call GA”. Chain field sign-off still open. Hosted/public not claimed. |
| **Desktop (Tauri)** | P1 | Partial | Production claim on site under Hot Lemon Pepper Sprinkles (`desktop-v1.0.0-beta.6`); **no GA label**. Fix download pin to beta.6. Balance/history QA + code signing still open. |
