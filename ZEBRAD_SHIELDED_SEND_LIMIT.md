...
**Sapling:** Scanning and witness tracking for **Sapling** use the same pattern: **`z_gettreestate`** Sapling `finalState` for checkpoints, **`SaplingWitnessTracker`** + incremental witnesses from block JSON, and anchor checks via **`get_sapling_tree_state`** (parsed from `z_gettreestate`). Sapling spends are built with **`zcash_primitives::transaction::builder::Builder`** and **`zcash_proofs::LocalTxProver::bundled()`** (Groth16). Recipient selection prefers **Orchard** when a UA includes both pools; otherwise **Sapling-only** UAs use the Sapling send path when Sapling notes are available.
