...
use nozy::{
    HDWallet, WalletStorage, ZebraClient, NoteScanner, 
    OrchardTransactionBuilder, NozyResult, NozyError,
};
...
async fn test_wallet_create_and_save() {
    ...
    let loaded_wallet = storage.load_wallet(password)
        .await
...
#[tokio::test]
#[ignore] async fn test_wallet_restore_from_mnemonic() {
    setup_test_env();
    ...
    let restored_wallet = HDWallet::from_mnemonic(&mnemonic)
        .expect("Failed to restore wallet from mnemonic");
    
    assert_eq!(original_wallet.get_mnemonic(), restored_wallet.get_mnemonic());
    
    
    let addr1 = original_wallet.generate_orchard_address(0, 0)
        .expect("Failed to generate address");
    let addr2 = restored_wallet.generate_orchard_address(0, 0)
        .expect("Failed to generate address");
...
async fn test_address_generation() {
    ...
    for i in 0..5 {
        let address = wallet.generate_orchard_address(0, i)
            .expect("Failed to generate address");
...
async fn test_end_to_end_flow_create_scan_send() {
    ...
    let tip_height = client.get_block_count().await
        .expect("Failed to get block count");
    ...
    let (scan_result, spendable_notes) = scanner.scan_notes(
        Some(start_height),
        Some(tip_height)
    ).await.expect("Failed to scan notes");
    
    println!("✅ Scanned {} notes, {} spendable, balance: {:.8} ZEC",
    ...
    let recipient_address = recipient_wallet.generate_orchard_address(0, 0)
    ...
    println!("✅ Transaction built: {} bytes, TXID: {}",
...
async fn test_note_scanning() {
    ...
    let (result, spendable) = scanner.scan_notes(
        Some(start_height),
        Some(tip_height)
    ).await.expect("Failed to scan notes");
    
    println!("✅ Scanned {} blocks, found {} notes, {} spendable",
...
async fn test_wallet_storage_with_password() {
    ...
    wallet.set_password(password).expect("Failed to set password");
...
async fn test_multiple_address_generation() {
    ...
    for i in 0..10 {
        let addr = wallet.generate_orchard_address(0, i)
            .expect("Failed to generate address");
        addresses.push(addr);
    }
    ...
    for i in 0..addresses.len() {
        for j in (i+1)..addresses.len() {
            assert_ne!(addresses[i], addresses[j], 
                "Addresses at index {} and {} should be different", i, j);
        }
    }
